...
Many formation mechanisms have been proposed for spicules, including reconnection, \emph{p}-mode driving and applying accretion disk models to the plasma sinking at the inter-granular lanes, for reviews see \cite{Sterling2000} and \cite{Zaqara_Erdelyi2009}.
...
Any contribution in terms of solar wind acceleration or heat transfer would be scaled up by their extremely high number density.
...
The authors demonstrate that the high velocities and extents of the RRB's/RBE's are very similar to those previously cited byt \cite{Beckers1972} and \cite{DePontieu2007}. 

Higher in the chromosphere, macrospicules (MSs) are generated, despite their origin laying low in the chromosphere, MSs extend through to the transition region and into the corona. 
...
In the following sections we comprehensively discuss the physical properties of a case study. In \cref{sec:obs_sect} we present the observations we are using. 
...
Then move on to a doppler analysis of the jet in \cref{sec:dop_shift_sect}.
Lastly, attempt to quantify the effect of the jet on the atmosphere in \cref{sec:temp_map_sect} before making our conclusions.
...
We used the H$\alpha$ filter, core line $656.28$ nm with $35$ slit increments from the core covering a $.32$ nm range, $-0.2$ nm and $+0.12$ nm, further processed using te Multi-Object Multi-Frame Blind Deconvolution (MOMBFD \cite{vanNoort2005}).
...
It also appears in EUVI $30.4$ nm, as a dark feature, therefore we can't assume there is no emission in the higher lines.
...
Evidently, the maximum extent of this feature is in AIA/SDO $30.4$ nm and maximises at $12.6$ Mm although the measurements in H$\alpha$ may exceed this were it to be fully visible.
