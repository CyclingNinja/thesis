...
Where as measurement error on the SST/CRISP images is $\pm0.0885$ Mm, thus, we are more inclined to believe the measurement according to CRISP.
...
The jet appears as a dark fibril like feature originating at approximately $-570''$ to $-385''$ longitude , and extending across the disk to a length of $45.1$ arcsec.
...
The result of applying the above method to the SST/CRISP spectroscopic dataset is presented in \cref{fig:dop-exam}.
...
multi_db = texfigure.MultiFigure(2, 2, reference="demo-bound")
...
for i, im in enumerate(ims):
	fig = plt.figure(figsize=texfigure.figsize(pytex, scale=0.5, height_ratio=1.3))
	ax = plt.gca()
...
The jet is located around the $3$rd and $4$th pixels of the x axis, and as we would expect we see a moderate dip, when compared to the surroundings. 
...
We observe a jet-like feature at the solar limb on $21$st June 2012 at $07:30:00$ UTC on the solar limb, centred at $910$, $234$ solar helioprojective co-ordinates.
We have multiple methods with which to view the feature, SST/CRISP, the range of SDO/AIA coronal wavelengths and STEREO-A/SECCHI.
