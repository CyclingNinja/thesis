...
class SST_feature():
    ...
    def __str__(self):
        ...
        astr = """ A Spicule object
                 ...
                 has limb: %f
                 has location: %s
                 has points: %s """%(str(self.frames), str([i.time for i in self.times]),
...
# define the measurements
    def length(self, frame):
        ...
        return (np.sqrt((B[0] - A[0])**2 + (B[1] - A[1])**2)*0.725*0.059)
    ...
    def width(self, frame):
        ...
        return (np.sqrt((C[0] - D[0])**2 + (C[1] - D[1])**2)*0.725*0.059)
